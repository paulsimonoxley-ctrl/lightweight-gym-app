...
const theme = {
    bg: '#0a0b0d', surface: 'rgba(18,18,24,0.85)', border: 'rgba(120,80,200,0.25)',
...
export function WorkoutDatabase({ targetWeight, targetReps, onLogComplete }: WorkoutDatabaseProps) {
    ...
    const handleLog = () => {
        ...
        if (w > targetWeight) setFeedback({ message: `🔥 Beast mode. +${(w - targetWeight).toFixed(1)} ${weightUnit} over target. Progressive overload achieved.`, type: 'success' });
        ...
        else setFeedback({ message: 'Target locked. Increase weight next session for progressive overload.', type: 'neutral' });
        onLogComplete(w, r);
    };

    const inputStyle: React.CSSProperties = {
        background: 'rgba(5,5,8,0.8)', border: '1px solid rgba(120,80,200,0.3)', borderRadius: '12px',
        padding: '10px 12px', textAlign: 'center', color: theme.textPrimary,
        fontSize: '18px', fontFamily: "'Space Grotesk', monospace", fontWeight: 700,
    ...
    const labelStyle: React.CSSProperties = {
        ...
        textTransform: 'uppercase' as const, color: theme.textDim, textAlign: 'center', marginTop: '6px',
    };
        ...
        <div>
            <div style={{ background: 'rgba(5,5,8,0.5)', border: `1px solid ${theme.border}`, borderRadius: '16px', overflow: 'hidden', marginBottom: '20px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', padding: '14px 20px', fontSize: '14px', color: theme.textDim, fontWeight: 400, borderBottom: '1px solid rgba(120,80,200,0.08)' }}>
                    ...
                    <span style={{ textAlign: 'center', fontFamily: "'Space Grotesk', monospace" }}>{targetWeight} <span style={{ opacity: 0.4, fontSize: '11px' }}>{weightUnit.toUpperCase()}</span> × {targetReps}</span>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', padding: '16px 20px', fontSize: '14px', color: theme.textPrimary, fontWeight: 600, alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <History size={15} style={{ color: theme.violet }} />
                        <span>Current Set</span>
                    </div>
                    <div style={{ textAlign: 'center', fontFamily: "'Space Grotesk', monospace", color: '#a78bfa' }}>
                        {targetWeight} <span style={{ opacity: 0.4, fontSize: '11px' }}>{weightUnit.toUpperCase()}</span> × {targetReps}
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', alignItems: 'center' }}>
                        <div>
                            <input type="number" value={actualWeight} onChange={e => setActualWeight(e.target.value)}
                                onFocus={e => { e.target.style.borderColor = theme.violet; e.target.style.boxShadow = `0 0 14px ${theme.violetGlow}`; }}
                                onBlur={e => { e.target.style.borderColor = 'rgba(120,80,200,0.3)'; e.target.style.boxShadow = 'none'; }}
                                style={inputStyle} />
                            <label style={labelStyle}>{weightUnit.toUpperCase()}</label>
                        </div>
                        <span style={{ color: theme.textDim }}>×</span>
                        <div>
                            <input type="number" placeholder="0" value={actualReps} onChange={e => setActualReps(e.target.value)}
                                onFocus={e => { e.target.style.borderColor = theme.crimson; e.target.style.boxShadow = `0 0 14px ${theme.crimsonGlow}`; }}
                                onBlur={e => { e.target.style.borderColor = 'rgba(120,80,200,0.3)'; e.target.style.boxShadow = 'none'; }}
                                style={{ ...inputStyle, borderColor: actualReps ? 'rgba(232,0,61,0.3)' : 'rgba(120,80,200,0.3)' }} />
