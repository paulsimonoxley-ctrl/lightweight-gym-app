        ...
        <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: '18px', padding: '20px', marginBottom: '24px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '4px' }}>
                {cells.map((day, i) => {
                    ...
                    return (
                        <button key={day} onClick={() => !isPast && onSelectDate(dateStr)} style={{ aspectRatio: '1', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', borderRadius: '8px', border: isSelected ? `1px solid ${T.borderGlow}` : isToday ? `1px solid rgba(124,58,237,0.35)` : '1px solid transparent', background: isSelected ? 'rgba(124,58,237,0.18)' : isToday ? 'rgba(124,58,237,0.08)' : 'transparent', cursor: isPast ? 'default' : 'pointer', opacity: isPast ? 0.35 : 1, padding: 0 }}>
...
export function ScheduleScreen() {
    ...
    const upcoming = commitments.filter(c => c.scheduled_date >= today);
    const past = commitments.filter(c => c.scheduled_date < today);
        ...
        <Layout title="SCHEDULE" subtitle="Commit & Consist">
            <div style={{ padding: '24px 20px', maxWidth: '600px', margin: '0 auto' }}>
                ...
                <h2 style={{ fontFamily: "'Oswald', sans-serif", fontSize: '13px', fontWeight: 600, letterSpacing: '0.18em', textTransform: 'uppercase', color: T.textMuted, margin: '0 0 14px', display: 'flex', alignItems: 'center', gap: '8px', justifyContent: 'space-between' }}>
                    ...
                    <button onClick={() => openModal(today)} style={{ display: 'flex', alignItems: 'center', gap: '4px', background: 'rgba(124,58,237,0.1)', border: '1px solid rgba(124,58,237,0.3)', borderRadius: '8px', padding: '5px 10px', color: '#a78bfa', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}>
                        <Plus size={12} /> Add
                    </button>
                </h2>
                        ...
                        {upcoming.map(c => {
                            ...
                            return (
                                <div key={c.id} style={{ background: T.surface, border: `1px solid ${color}44`, borderRadius: '16px', padding: '16px 18px', display: 'flex', alignItems: 'center', gap: '14px', position: 'relative', overflow: 'hidden' }}>
                                    ...
                                    <button onClick={() => startCommitment(c)} style={{ display: 'flex', alignItems: 'center', gap: '5px', background: `linear-gradient(135deg, ${T.violet}, ${T.crimson})`, border: 'none', borderRadius: '10px', padding: '8px 14px', color: 'white', fontSize: '12px', fontWeight: 700, cursor: 'pointer' }}><Play size={12} /> Start</button>
                ...
                {past.length > 0 && (
                    <div style={{ opacity: 0.5 }}>
                        <h3 style={{ fontFamily: "'Oswald', sans-serif", fontSize: '11px', fontWeight: 600, letterSpacing: '0.18em', textTransform: 'uppercase', color: T.textDim, margin: '0 0 10px' }}>Past Commitments</h3>
                ...
                <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', background: 'rgba(5,5,8,0.9)', backdropFilter: 'blur(16px)' }}>
                    <div style={{ background: 'rgba(14,12,24,1)', border: `1px solid ${T.borderGlow}`, borderRadius: '24px 24px 0 0', width: '100%', maxWidth: '600px', padding: '28px 24px 32px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
                            {workouts.map(w => (
                                <button key={w.id} onClick={() => setSelectedWorkout(w)} style={{ background: selectedWorkout?.id === w.id ? `${w.color}18` : T.surface, border: selectedWorkout?.id === w.id ? `1px solid ${w.color}88` : `1px solid ${T.border}`, borderRadius: '14px', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: '14px', cursor: 'pointer', textAlign: 'left' }}>
