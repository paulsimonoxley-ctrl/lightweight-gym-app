        ...
        <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: '18px', padding: '20px', marginBottom: '24px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '4px' }}>
                {cells.map((day, i) => {
                    ...
                    return (
                        <div key={day} style={{ aspectRatio: '1', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', borderRadius: '8px', position: 'relative', background: isToday ? 'rgba(124,58,237,0.12)' : 'transparent', border: isToday ? `1px solid rgba(124,58,237,0.35)` : '1px solid transparent' }}>
                            <span style={{ fontSize: '12px', fontWeight: isToday ? 700 : 400, color: isToday ? '#a78bfa' : T.textMuted }}>{day}</span>
                            {color && (
                                <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: color, boxShadow: `0 0 6px ${color}`, marginTop: '2px' }} />
                            )}
                        </div>
                    );
                })}
            ...
            <div style={{ padding: '24px 20px', maxWidth: '600px', margin: '0 auto' }}>
                ...
                <div style={{ marginTop: '32px', paddingTop: '24px', borderTop: `1px solid ${T.border}` }}>
                    <button
                        ...
                        style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', background: 'rgba(232,0,61,0.06)', border: '1px solid rgba(232,0,61,0.25)', borderRadius: '14px', padding: '14px', color: T.crimson, fontSize: '14px', fontWeight: 600, cursor: 'pointer', letterSpacing: '0.05em', transition: 'all 0.2s' }}
